...
export function getDeityFavorability(chara: Character, deity: Deity): string {
  ...
  for (let i = 0; i < deity.Aspects.length; i++) {
    ...
    tempCalc = tempCalc + stats[`${deity.Aspects[i].tagName}Affinity`] / 3;
    tempCalc = tempCalc - stats[`${deity.Aspects[i].tagName}Resist`] / 6;
  ...
  return 'Favorability: ' + Math.floor(favCalc);
}
